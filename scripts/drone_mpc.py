...
class DroneMPC(object):
    def __init__(self, N):
        ...
        self.FLAT_STATES = 7
        self.FLAT_CTRLS = 4
        self.A = np.zeros((self.FLAT_STATES, self.FLAT_STATES), dtype=np.float16)
        ...
        self.xmin = np.array([0, 0, 0, -2, -2, -2, -np.Inf])
    ...
    def viewpoint_cost(self, xt, xt_target, target_pixel, phi):
        """
    ...
    def objective(U, self, t, x0, target_traj, target_pixel, phi0):
        ...
        viewpoint_cost = sum([self.viewpoint_cost(X[i], target_traj[i], target_pixel, phi0)
                              for i in range(self.N)])
        return 1 * control_cost + 10 * viewpoint_cost
